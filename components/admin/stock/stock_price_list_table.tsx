    ...
    try {
      const res = await fetch(`/api/stock/delete_stock`, {
        ...
        headers: {
          "Content-Type": "application/json",
          authorization:
            "NEXT_PUBLIC_API_SECRET_KEY 9ddf045fa71e89c6d0d71302c0c5c97e",
        },
      });
                    ...
                    <TableRow key={row.tickerSymbol}>
                      ...
                      <TableCell align="right">
                        <Button
                          variant="text"
                          color="error"
                          onClick={() => updatePrices(row.tickerSymbol)}
                        >
                          Get Prices
                        </Button>
                      </TableCell>
                      <TableCell align="right">
                        <Button
                          variant="text"
                          color="error"
                          onClick={() => deleteStock(row.tickerSymbol)}
                        >
                          Delete
                        </Button>
                      </TableCell>
...
async function get_all_stocks_api() {
  return fetch("/api/stock/get_all_stocks_with_hsp")
    .then((res) => {
      if (res.status === 200) {
        return res
          .json()
